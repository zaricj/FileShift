...
from PySide6.QtCore import Qt, QSettings, QFile, QTextStream, QObject, Signal, QThread
...
class ProgressbarThread(QObject):
    finished = Signal()
    update_progress = Signal(int)
    update_output = Signal(str)
    update_statusbar = Signal(str, int)
    update_statusbar_style = Signal(str)
    show_error = Signal(str, str)  
    
    def __init__(self, file_path, destination, file_content):
        super().__init__()
        # Store only the values, not widget references
    ...
    def run(self):
        ...
        if not self.destination:
            self.update_statusbar_style.emit("color: red")
            self.update_statusbar.emit("Please provide a destination directory.", 10000)
            self.update_output.emit("ERROR: Destination directory has not been set.")
            self.finished.emit()
            return
        else:
            if not self.file_content:
                self.update_statusbar_style.emit("color: red")
                self.update_statusbar.emit("File content display is empty, hence nothing to move.")
                return
            else:
                self.update_statusbar_style.emit("color: #2cde85")
                self.update_statusbar.emit("Using the displayed file content.", 10000)
                ...
                try:
                    ...
                    lines = self.clean_paths_in_line(self.file_content)
                    ...
                    for line in lines:
                        ...
                        if file_to_move:
                            ...
                            current_destination = os.path.join(self.destination, sub_dir)
                            ...
                            try:
                                ...
                                self.update_output.emit(f"Moved <span style='color:rgb(39, 124, 236)'>{file_to_move}</span> to <span style='color: green'>{current_destination}</span>")
                                self.update_statusbar.emit(f"Moved {line_count}/{total_lines} files.", 10000)
                            except FileNotFoundError:
                                self.update_output.emit(f"<span style='color: orange'>WARN: {file_to_move}</span> not found, skipping.")
                                warn_count += 1
                                continue
                            except shutil.Error as e:
                                self.update_output.emit(f"<span style='color: red'>ERROR: {e}</span>")
                                err_count += 1
                                continue
                            line_count += 1
                    self.update_output.emit(f"\n{task_completed_message}\n")
                    if err_count > 0:
                        self.update_output.emit(f"<span style='color: red'><strong>ERROR:</strong></span> {err_count} files failed to move.")
                    if warn_count > 0:
                        self.update_output.emit(f"<span style='color: orange'><strong>WARNING:</strong></span> {warn_count} files were not found.")
                except Exception as e:
                    self.update_output.emit(f"<span style='color: red'>FATAL ERROR: {e}</span>")
    
    def clean_paths_in_line(self, file_content_display_text):
        try:
            file_content_display_text = self.file_content
            ...
            self.show_error.emit("Error", f"An error occurred while cleaning high commas ' in the paths: {str(ex)}")
            return []
...
class MainWindow(QMainWindow):
    ...
    def move_files(self):
        # Get the values you need from widgets
        self.progressbar.setVisible(True)
        file_path = self.file_path_input.text()
        destination = self.destination_input.text()
        file_content = self.file_content_display.toPlainText()
        
        # Create new instance of worker with just the values
        ...
        # Connect progress updates and style changes
        ...
        self.worker.update_output.connect(self.program_output.append)
        self.worker.update_statusbar_style.connect(self.update_statusbar_style)
        self.worker.show_error.connect(self.show_error_message)  # Connect error signal
        
        # Start the thread
        self.thread.start()
    
    def update_statusbar_style(self, style):
